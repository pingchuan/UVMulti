...
# KavSir-SEG Dataset
# KavSir-SEG Dataset
class BenthicVideoDataset_SUP(Dataset):
    def __init__(self, root, data2_dir, mode='train', transform=None):
        super(BenthicVideoDataset_SUP, self).__init__()
        self.root = root
        self.mode = mode
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            if self.transform:
                ...
                img_s1 = self.apply_mosaic(self, img_s1, label_image, Q1, size=(512, 512))
    ...
    def apply_mosaic(self, img_s1, label_img, Q1, size=(512, 512)):

        new_size = (size[0] // 2, size[1] // 2)
        import cv2
        fg_sequence_path, fg_frame, fg_gt_path = random.sample(Q1, 3)

        # 加载前景图像和标签
        fg_image = self.load_image(fg_sequence_path, fg_frame)
        fg_label = self.load_label(fg_gt_path, fg_frame)
        fg_image = [img_s1] + fg_image  # 包括原始图像和随机选择的三张图像
        fg_label = [label_img] + fg_label  # 包括原始标签和随机选择的标签
        # 调整图像尺寸
        resized_imgs = [cv2.resize(img, new_size) for img in fg_image]
        resized_labels = [cv2.resize(label, new_size) for label in fg_label]

        # 创建空的图像和标签容器
        mosaic_img = np.zeros((size[0], size[1], 3), dtype=np.uint8)
        mosaic_label = np.zeros((size[0], size[1]), dtype=np.uint8)

        # 拼接图像，2x2布局
        mosaic_img[:size[0] // 2, :size[1] // 2] = resized_imgs[0][:size[0] // 2, :size[1] // 2]
        mosaic_img[:size[0] // 2, size[1] // 2:] = resized_imgs[1][:size[0] // 2, size[1] // 2:]
        mosaic_img[size[0] // 2:, :size[1] // 2] = resized_imgs[2][size[0] // 2:, :size[1] // 2]
        mosaic_img[size[0] // 2:, size[1] // 2:] = resized_imgs[3][size[0] // 2:, size[1] // 2:]

        # 拼接标签，2x2布局
        mosaic_label[:size[0] // 2, :size[1] // 2] = resized_labels[0][:size[0] // 2, :size[1] // 2]
        mosaic_label[:size[0] // 2, size[1] // 2:] = resized_labels[1][:size[0] // 2, size[1] // 2:]
        mosaic_label[size[0] // 2:, :size[1] // 2] = resized_labels[2][size[0] // 2:, :size[1] // 2]
        mosaic_label[size[0] // 2:, size[1] // 2:] = resized_labels[3][size[0] // 2:, size[1] // 2:]
