...
class BenthicVideoDataset_Enh_CZ(Dataset):
    def __init__(self, root, data2_dir, mode='train', transform=None):
        super(BenthicVideoDataset_Enh_CZ, self).__init__()
        self.root = root
        self.mode = mode
        ...
        for video in self.video_dirs:
            ...
            # 选择要处理的帧：首帧、每15帧和末尾帧
            selected_frames = [frames[0]]  # 添加首帧

            # 添加每15帧的帧
            for i in range(1, len(frames) - 1):  # 跳过首帧和末尾帧
                if i % 15 == 0:
