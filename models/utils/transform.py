...
class ToTensor1(object):
    ...
    def __call__(self, data):
        ...
        # 将图像转换为张量
        data['image'] = F.to_tensor(image)

        if label is not None:
            # 将标签转换为长整型张量，并确保维度正确
            label = np.array(label)  # 转换为 numpy 数组
            data['label'] = torch.tensor(label, dtype=torch.long)  # 转换为长整型张量
            data['label'] = data['label'].unsqueeze(0)  # 添加通道维度

        return data

class ToTensor_1(object):
    def __init__(self, train=False):
        self.train = train
    def __call__(self, data):
        image, label = data['image'], data['label']
        ...
        return {'image': F.to_tensor(image), 'label': F.to_tensor(label), 'depth': F.to_tensor(depth)}
...
class Resize2(object):
    ...
    def __init__(self, size, train=False):
        ...
        self.train = train
    def __call__(self, data):
        image,img1,img11, label = data['image'], data['img1'],data['img11'],data['label']
        return {'image': F.resize(image, self.size), 'img1': F.resize(img1, self.size),'img11': F.resize(img11, self.size),'label': F.resize(label, self.size,interpolation=Image.NEAREST)}
class Resize_1(object):
    ...
    def __call__(self, data):
        image, label = data['image'], data['label']
        depth = data['depth']
        return {'image': F.resize(image, self.size), 'label': F.resize(label, self.size), 'depth': F.resize(depth, self.size)}
...
class RandomHorizontalFlip2(object):
    ...
    def __call__(self, data):
        ...
        label = label.convert("L")
        if random.random() < self.p:
            return {'image': F.hflip(image),'img1': F.hflip(img1),'img11': F.hflip(img11), 'label': F.hflip(label)}
        return data
...
class RandomVerticalFlip2(object):
    ...
    def __call__(self, data):
        ...
        label = label.convert("L")
        if random.random() < self.p:
            return {'image': F.vflip(image),'img1': F.vflip(img1),'img11': F.vflip(img11), 'label': F.vflip(label)}
    ...
    def __call__(self, data):
        ...
        if random.random() < 0.5:
            ...
            return {'image': F.rotate(image, angle, self.resample, self.expand, self.center),
                    ...
                    'img11': F.rotate(img11, angle, self.resample, self.expand, self.center),
                    'label': F.rotate(label, angle, self.resample, self.expand, self.center)}
