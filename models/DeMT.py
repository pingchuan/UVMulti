...
class BaseHead(nn.Module):
    def __init__(self):
        ...
        self.idx_to_planes = {1: 64, 2: 128, 3: 256, 4: 512}
...
class DemtHead(BaseHead):
    ...
    def __init__(self):
        ...
        self.bottleneck = nn.ModuleDict({t: ConvBNReLU(dim_,
                                                                   out_channels,
                                                                   kernel_size=3,
        ...
        self.task_querys = nn.ModuleList([nn.MultiheadAttention(embed_dim=dim_, num_heads=4, dropout=0.,batch_first=True)  for t in range (len(self.tasks))])
    ...
    def forward(self, inp):
        ...
        #final_pred = {t: nn.functional.interpolate(
         #               final_pred[t], size=[320,320], mode='bilinear', align_corners=False) for t in self.tasks}
        for t in self.tasks:
            #out[t] = F.interpolate(self.heads[t](multi_scale_features[t]),[320, 320], mode='bilinear')
            self.outputs[t] = F.interpolate(final_pred[t], [256, 256], mode='bilinear')
