...
def resnet_multiimage_input(num_layers, pretrained=False, num_input_images=1):
    ...
    if pretrained:
        ...
        # We exclude the 'conv1' weight from the pretrained model and update others
        ...
        # Now, manually adjust the first convolution layer (conv1)
        pretrained_conv1_weight = pretrained_model.conv1.weight

        # Create a new weight tensor for conv1 with the correct number of input channels
        new_weight = torch.cat([pretrained_conv1_weight] * num_input_images, dim=1)

        # Normalize the new conv1 weights by dividing by num_input_images
        model.conv1.weight = torch.nn.Parameter(new_weight / num_input_images)
    return model
    #if pretrained:
...
class DecoderBN_50(nn.Module):
    def __init__(self, num_features=2048, num_classes=1, bottleneck_features=512):
        super(DecoderBN_50, self).__init__()
        features = int(num_features)
...
class ResnetEncoderDecoder(nn.Module):
    def __init__(self, num_layers=50, num_features=512, model_dim=32):
        ...
        #self.decoder = DecoderBN(num_features=num_features, num_classes=model_dim, bottleneck_features=512)
        self.decoder = DecoderBN_50(num_features=num_features, num_classes=model_dim, bottleneck_features=2048)
