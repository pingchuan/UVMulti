...
class Res2Net(nn.Module):
    ...
    def __init__(self, block, layers, baseWidth=26, scale=4,input_channels=3, num_classes=1000):
        self.inplanes = 64
        ...
        self.conv1_1 = nn.Conv2d(input_channels, self.inplanes,
                               kernel_size=7, stride=2, padding=3, bias=False)  # stried=2改为1
    ...
    def forward_first_conv(self, x):
        ...
        #x = self.conv1(x) #masnet原始的
        x = self.conv1_1(x)
        x = self.bn1(x)
        x = self.relu(x)
        return x

    def forward_first_conv_depth(self, x):
        # be aware that maxpool still needs to be applied after this function
        # and before forward_layer1()
...
def res2net50_v1b_26w_4s(pretrained=False, input_channels=3,**kwargs):
    """Constructs a Res2Net-50_v1b_26w_4s lib.
    Args:
        pretrained (bool): If True, returns a lib pre-trained on ImageNet
    """
    model = Res2Net(Bottle2neck, [3, 4, 6, 3], baseWidth=26, scale=4,input_channels=input_channels, **kwargs)
    if pretrained:
        # model_state = torch.load('./models/res2net50_v1b_26w_4s-3cf99910.pth')
        # model.load_state_dict(model_state)
        #model.load_state_dict(model_zoo.load_url(model_urls['res2net50_v1b_26w_4s']))
        weights = model_zoo.load_url(model_urls['res2net50_v1b_26w_4s'])
...
def res2net101_v1b_26w_4s(pretrained=False, input_channels=3,**kwargs):
    """Constructs a Res2Net-50_v1b_26w_4s lib.
    Args:
        pretrained (bool): If True, returns a lib pre-trained on ImageNet
    """
    ...
    if pretrained:
        #model.load_state_dict(model_zoo.load_url(model_urls['res2net101_v1b_26w_4s']))
        weights = model_zoo.load_url(model_urls['res2net101_v1b_26w_4s'])
